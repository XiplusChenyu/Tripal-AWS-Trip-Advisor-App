...
for i in range(len(temp["Quotes"])):
    route += [{"price":temp["Quotes"][i]["MinPrice"],"carrier":carrier_dic[temp["Quotes"][i]["OutboundLeg"]["CarrierIds"][0]],"departure_date":temp["Quotes"][i]["OutboundLeg"]["DepartureDate"],"back_date":temp["Quotes"][i]["InboundLeg"]["DepartureDate"]}]
# print(route)
